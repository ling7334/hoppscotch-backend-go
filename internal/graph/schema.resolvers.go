...
func (r *infraResolver) UserInfo(ctx context.Context, obj *dto.Infra, userUID string) (u *model.User, err error) {
	db := GetPreloadedDB(r.DB, ctx)
	err = db.Where(`"uid"=?`, userUID).First(u).Error
	return
}

func (r *infraResolver) AllUsersV2(ctx context.Context, obj *dto.Infra, searchString *string, skip *int, take *int) (u []*model.User, err error) {
	// panic(fmt.Errorf("not implemented: AllUsersV2 - allUsersV2"))
	db := GetPreloadedDB(r.DB, ctx)
	if skip != nil {
		db = db.Offset(*skip)
	}
	db = db.Limit(getLimit(take))
	if searchString != nil && strings.TrimSpace(*searchString) != "" {
		db = db.Where(`"displayName" LIKE ? OR "email" LIKE ?`, "%"+*searchString+"%", "%"+*searchString+"%")
	}
	err = db.Find(&u).Error
	return
}

func (r *infraResolver) InvitedUsers(ctx context.Context, obj *dto.Infra, skip *int, take *int) (iu []*model.InvitedUser, err error) {
	// panic(fmt.Errorf("not implemented: InvitedUsers - invitedUsers"))
	db := GetPreloadedDB(r.DB, ctx)
	if skip != nil {
		db = db.Offset(*skip)
	}
	err = db.Limit(getLimit(take)).Find(&iu).Error
	return
}
...
func (r *infraResolver) MembersCountInTeam(ctx context.Context, obj *dto.Infra, teamID string) (count int64, err error) {
...
func (r *infraResolver) CollectionCountInTeam(ctx context.Context, obj *dto.Infra, teamID string) (count int64, err error) {
...
func (r *infraResolver) RequestCountInTeam(ctx context.Context, obj *dto.Infra, teamID string) (count int64, err error) {
...
func (r *infraResolver) EnvironmentCountInTeam(ctx context.Context, obj *dto.Infra, teamID string) (count int64, err error) {
...
func (r *infraTokenResolver) LastUsedOn(ctx context.Context, obj *model.InfraToken) (*time.Time, error) {
	// panic(fmt.Errorf("not implemented: LastUsedOn - lastUsedOn"))
...
func (r *mutationResolver) UpdateDisplayName(ctx context.Context, updatedDisplayName string) (*model.User, error) {
	// panic(fmt.Errorf("not implemented: UpdateDisplayName - updateDisplayName"))
	user, ok := ctx.Value(mw.ContextKey("operator")).(*model.User)
	if !ok {
		return nil, ex.ErrBugAuthNoUserCtx
	}

	err := r.DB.Model(user).Clauses(clause.Returning{}).Update("displayName", updatedDisplayName).Error
	return user, err
}
...
func (r *mutationResolver) UpdateInfraConfigs(ctx context.Context, infraConfigs []*dto.InfraConfigArgs) (c []*model.InfraConfig, err error) {
	...
	for _, cf := range infraConfigs {
		...
		cfg, err = Update(r.DB, cf.Name, cf.Value, false)
		if err != nil {
			tx.Rollback()
			return
		} else {
			c = append(c, &cfg)
			InfraConfigUpdateSub.Publish(cf)
		}
	}
	tx.Commit()
	return
...
func (r *mutationResolver) ToggleAnalyticsCollection(ctx context.Context, status dto.ServiceStatus) (bool, error) {
	// panic(fmt.Errorf("not implemented: ToggleAnalyticsCollection - toggleAnalyticsCollection"))
	_, err := Update(r.DB, dto.InfraConfigEnumAllowAnalyticsCollection, status.String(), false)
	if err != nil {
		return false, err
	}
	InfraConfigUpdateSub.Publish(&dto.InfraConfigArgs{
		Name:  dto.InfraConfigEnumAllowAnalyticsCollection,
		Value: status.String(),
	})
	return true, nil
}
...
func (r *mutationResolver) ResetInfraConfigs(ctx context.Context) (bool, error) {
	...
	RESET_EXCLUSION_LIST := []string{
		dto.InfraConfigEnumIsFirstTimeInfraSetup.String(),
		dto.InfraConfigEnumAnalyticsUserID.String(),
		dto.InfraConfigEnumAllowAnalyticsCollection.String(),
	}

	defaultConfigs := getDefaultInfraConfigs(RESET_EXCLUSION_LIST)
	toReset := []string{}
	newConfigs := []*model.InfraConfig{}
	for k, v := range defaultConfigs {
		toReset = append(toReset, k.String())
		newConfigs = append(newConfigs, &model.InfraConfig{
			Name:  k.String(),
			Value: &v,
		})
	}
	...
	if err := tx.Model(&model.InfraConfig{}).Delete(`"name" IN ?`, toReset).Error; err != nil {
		tx.Rollback()
		return false, ex.ErrInfraConfigResetFailed
	}

	if err := tx.Create(&newConfigs).Error; err != nil {
		tx.Rollback()
		return false, ex.ErrInfraConfigResetFailed
	}
...
func (r *mutationResolver) EnableAndDisableSso(ctx context.Context, providerInfo []*dto.EnableAndDisableSSOArgs) (bool, error) {
	...
	cfgs, err := GetInfraConfigsMap(r.DB)
	if err != nil {
		return false, err
	}
	allowedAuthProvider, ok := cfgs[string(dto.InfraConfigEnumViteAllowedAuthProviders)]
	if !ok || allowedAuthProvider == "" {
		allowedAuthProvider = os.Getenv("VITE_ALLOWED_AUTH_PROVIDERS")
	}
	allowedAuthProviders := strings.Split(allowedAuthProvider, ",")
	updatedAuthProviders := allowedAuthProviders

	infraConfigMap := map[string]string{}
	var infraConfigs []model.InfraConfig
	if err := r.DB.Find(&infraConfigs).Error; err != nil {
		return false, err
	}
	for _, c := range infraConfigs {
		infraConfigMap[c.Name] = *c.Value
	}

	for _, p := range providerInfo {
		if p.Status == dto.ServiceStatusEnable {
			if !isServiceConfigured(p.Provider, infraConfigMap) {
				return false, ex.ErrInfraConfigServiceNotConfigured
			}
			updatedAuthProviders = append(updatedAuthProviders, string(p.Provider))
		} else if p.Status == dto.ServiceStatusDisable {
			dupSlice := updatedAuthProviders
			updatedAuthProviders = []string{}
			for _, ap := range dupSlice {
				if ap != string(p.Provider) {
					updatedAuthProviders = append(updatedAuthProviders, ap)
				}
			}
		}
	}
	updatedAuthProviders = removeDups(updatedAuthProviders)
	if len(updatedAuthProviders) == 0 {
		return false, ex.ErrAuthProviderNotSpecified
	}
	providerString := strings.Join(updatedAuthProviders, ",")
	if _, err := Update(r.DB, dto.InfraConfigEnumViteAllowedAuthProviders, providerString, false); err != nil {
		return false, err
	}
	InfraConfigUpdateSub.Publish(&dto.InfraConfigArgs{
		Name:  dto.InfraConfigEnumViteAllowedAuthProviders,
		Value: providerString,
	})
	return true, nil
}

func (r *mutationResolver) ToggleSMTP(ctx context.Context, status dto.ServiceStatus) (bool, error) {
	// panic(fmt.Errorf("not implemented: ToggleSMTP - toggleSMTP"))
	tx := r.DB.Begin()
	if _, err := Update(tx, dto.InfraConfigEnumMailerSMTPEnable, status.String(), false); err != nil {
		tx.Rollback()
		return false, err
	}
	InfraConfigUpdateSub.Publish(&dto.InfraConfigArgs{
		Name:  dto.InfraConfigEnumMailerSMTPEnable,
		Value: status.String(),
	})
	if status == dto.ServiceStatusDisable {
		if _, err := r.EnableAndDisableSso(ctx, []*dto.EnableAndDisableSSOArgs{
			{
				Provider: dto.AuthProviderEmail,
				Status:   status,
			},
		}); err != nil {
			tx.Rollback()
...
func (r *mutationResolver) ToggleUserHistoryStore(ctx context.Context, status dto.ServiceStatus) (bool, error) {
	// panic(fmt.Errorf("not implemented: ToggleUserHistoryStore - toggleUserHistoryStore"))
	if _, err := Update(r.DB, dto.InfraConfigEnumUserHistoryStoreEnabled, status.String(), false); err != nil {
		return false, err
	}
	InfraConfigUpdateSub.Publish(&dto.InfraConfigArgs{
		Name:  dto.InfraConfigEnumUserHistoryStoreEnabled,
		Value: status.String(),
	})
	return true, nil
}
...
func (r *mutationResolver) RevokeUserInvitationsByAdmin(ctx context.Context, inviteeEmails []string) (bool, error) {
	// panic(fmt.Errorf("not implemented: RevokeUserInvitationsByAdmin - revokeUserInvitationsByAdmin"))
	err := r.DB.Delete(&model.InvitedUser{}, "inviteeEmail IN ?", inviteeEmails).Error
	return err == nil, err
}
...
func (r *mutationResolver) RemoveUsersByAdmin(ctx context.Context, userUIDs []string) (res []*dto.UserDeletionResult, err error) {
	// panic(fmt.Errorf("not implemented: RemoveUsersByAdmin - removeUsersByAdmin"))
	// var deletedUsers []model.User
	// err = r.DB.Where("user IN ?", userUIDs).Clauses(clause.Returning{}).Delete(&deletedUsers).Error
	// for _, u := range deletedUsers {
	// 	res = append(res, &dto.UserDeletionResult{
	// 		UserUID:      u.UID,
	// 		IsDeleted:    true,
	// 		ErrorMessage: nil,
	// 	})
	// 	UserDeletedSub.Publish(&u)
	// }
	for _, uid := range userUIDs {
		var deletedUser model.User
		err := r.DB.Where("uid=?", uid).Clauses(clause.Returning{}).Delete(&deletedUser).Error
		if err != nil {
			msg := err.Error()
			res = append(res, &dto.UserDeletionResult{
				UserUID:      uid,
			...
			UserDeletedSub.Publish(&deletedUser)
		}
	}
	return
}
...
func (r *mutationResolver) MakeUserAdmin(ctx context.Context, userUID string) (bool, error) {
	...
	var user model.User
	err := r.DB.Model(&user).Where("uid=?", userUID).Update("isAdmin", true).Error
	UserUpdatedSub.Publish(&user)
	return err == nil, err
}

func (r *mutationResolver) MakeUsersAdmin(ctx context.Context, userUIDs []string) (bool, error) {
	// panic(fmt.Errorf("not implemented: MakeUsersAdmin - makeUsersAdmin"))
	err := r.DB.Model(&model.User{}).Where("uid IN ?", userUIDs).Update("isAdmin", true).Error
	return err == nil, err
}

func (r *mutationResolver) UpdateUserDisplayNameByAdmin(ctx context.Context, userUID string, displayName string) (bool, error) {
	// panic(fmt.Errorf("not implemented: UpdateUserDisplayNameByAdmin - updateUserDisplayNameByAdmin"))
	err := r.DB.Model(&model.User{}).Where("uid=?", userUID).Update("displayName", displayName).Error
	return err == nil, err
}
...
func (r *mutationResolver) DemoteUsersByAdmin(ctx context.Context, userUIDs []string) (bool, error) {
	// panic(fmt.Errorf("not implemented: DemoteUsersByAdmin - demoteUsersByAdmin"))
	err := r.DB.Model(&model.User{}).Where("uid IN ?", userUIDs).Update("isAdmin", false).Error
	return err == nil, err
}
...
func (r *mutationResolver) ChangeUserRoleInTeamByAdmin(ctx context.Context, userUID string, teamID string, newRole model.TeamAccessRole) (*model.TeamMember, error) {
...
func (r *mutationResolver) AddUserToTeamByAdmin(ctx context.Context, teamID string, role model.TeamAccessRole, userEmail string) (*model.TeamMember, error) {
...
func (r *mutationResolver) RevokeAllUserHistoryByAdmin(ctx context.Context) (bool, error) {
	// panic(fmt.Errorf("not implemented: RevokeAllUserHistoryByAdmin - revokeAllUserHistoryByAdmin"))
	err := r.DB.Delete(&model.UserHistory{}).Error
	return err == nil, err
}
...
func (r *mutationResolver) UpdateTeamAccessRole(ctx context.Context, teamID string, userUID string, newRole model.TeamAccessRole) (*model.TeamMember, error) {
...
func (r *mutationResolver) CreateTeamInvitation(ctx context.Context, teamID string, inviteeEmail string, inviteeRole model.TeamAccessRole) (*model.TeamInvitation, error) {
...
func (r *mutationResolver) DuplicateTeamCollection(ctx context.Context, collectionID string) (bool, error) {
	// panic(fmt.Errorf("not implemented: DuplicateTeamCollection - duplicateTeamCollection"))
	coll := &model.TeamCollection{}
	if err := r.DB.First(coll, "id=?", collectionID).Error; err != nil {
		return false, err
	}
	dupColl := &model.TeamCollection{
		ID:         cuid.New(),
		ParentID:   coll.ParentID,
		TeamID:     coll.TeamID,
		Title:      coll.Title + " Copy",
		Data:       coll.Data,
		OrderIndex: coll.OrderIndex + 1,
	}

	if err := r.DB.Create(dupColl).Error; err != nil {
		return false, err
	}
	TeamCollectionAddedSub.Publish(dupColl)
	return true, nil
}
...
func (r *mutationResolver) ImportUserCollectionsFromJSON(ctx context.Context, jsonString string, reqType model.ReqType, parentCollectionID *string) (bool, error) {
	...
	var colls []model.UserCollection
	if err := json.Unmarshal([]byte(jsonString), &colls); err != nil {
		return false, err
	}
	AssignUserCollections(&colls, reqType, parentCollectionID)
	err := r.DB.Create(&colls).Error
	return err == nil, err
}
...
func (r *mutationResolver) DuplicateUserCollection(ctx context.Context, collectionID string, reqType model.ReqType) (bool, error) {
	// panic(fmt.Errorf("not implemented: DuplicateUserCollection - duplicateUserCollection"))
	user, ok := ctx.Value(mw.ContextKey("operator")).(*model.User)
	if !ok {
		return false, ex.ErrBugAuthNoUserCtx
	}
	var coll model.UserCollection
	if err := r.DB.Where(`id = ? AND "userUid" = ?`, collectionID, user.UID).First(&coll).Error; err != nil {
		return false, ex.ErrUserCollNotSameUser
	}
	tx := r.DB.Begin()
	if err := coll.Duplicate(tx); err != nil {
		tx.Rollback()
		return false, err
	}
	tx.Commit()
...
func (r *mutationResolver) CreateInfraToken(ctx context.Context, label string, expiryInDays *int) (*dto.CreateInfraTokenResponse, error) {
	// panic(fmt.Errorf("not implemented: CreateInfraToken - createInfraToken"))
	user, ok := ctx.Value(mw.ContextKey("operator")).(*model.User)
	if !ok {
		return nil, ex.ErrBugAuthNoUserCtx
	}
	rawToken := uuid.NewString()
	token := &model.InfraToken{
		ID:         cuid.New(),
		Label:      label,
		CreatorUID: user.UID,
		Token:      rawToken,
	...
	if expiryInDays != nil {
		expiry := time.Now().Add(time.Duration(*expiryInDays) * 24 * time.Hour)
		token.ExpiresOn = &expiry
	}
	if err := r.DB.Create(token).Error; err != nil {
		return nil, err
	}
	rsp := &dto.CreateInfraTokenResponse{
		Token: rawToken,
		Info:  token,
	}
	return rsp, nil
...
func (r *mutationResolver) RevokeInfraToken(ctx context.Context, id string) (bool, error) {
	// panic(fmt.Errorf("not implemented: RevokeInfraToken - revokeInfraToken"))
	err := r.DB.Delete(&model.InfraToken{}, "id=?", id).Error
	return err != nil, err
}

func (r *mutationResolver) SortUserCollections(ctx context.Context, parentCollectionID *string, sortOption dto.SortOptions) (bool, error) {
	// panic(fmt.Errorf("not implemented: SortUserCollections - sortUserCollections"))
	user, ok := ctx.Value(mw.ContextKey("operator")).(*model.User)
	if !ok {
		return false, ex.ErrBugAuthNoUserCtx
	}
	...
	base := GetPreloadedDB(tx, ctx).Where(
		`"userUid" = ? AND "parentID" IS ?`, user.UID, parentCollectionID).Clauses(
		clause.Locking{Strength: clause.LockingStrengthUpdate})
	if sortOption == dto.SortOptionsTitleAsc {
		base = base.Order(`"Title" ASC`)
	} else if sortOption == dto.SortOptionsTitleDesc {
		base = base.Order(`"Title" DESC`)
	} else {
		base = base.Order(`"orderIndex" DESC`)
	}
	if err := base.Find(&collections).Error; err != nil {
		return false, err
	}

	for i, coll := range collections {
		err := tx.Model(&model.UserCollection{}).Where(`id = ?`, coll.ID).Update(`"orderIndex"`, i+1).Error
		if err != nil {
			tx.Rollback()
			return false, ex.ErrUserCollReorderingFailed
		}
	}
	...
	if err := base.Find(&requests).Error; err != nil {
		return false, err
	}

	for i, req := range requests {
		err := tx.Model(&model.UserRequest{}).Where(`id = ?`, req.ID).Update(`"orderIndex"`, i+1).Error
		if err != nil {
			tx.Rollback()
			return false, ex.ErrUserRequestReorderingFailed
		}
	}

	if parentCollectionID != nil {
		UserChildCollectionsSortedSub.Publish(&dto.UserCollectionSortData{
			ParentCollectionID: parentCollectionID,
			SortOption:         sortOption,
		})
	} else {
		UserRootCollectionsSortedSub.Publish(&dto.UserCollectionSortData{
			ParentCollectionID: parentCollectionID,
			SortOption:         sortOption,
		})
	}
...
func (r *mutationResolver) SortTeamCollections(ctx context.Context, teamID string, parentCollectionID *string, sortOption dto.SortOptions) (bool, error) {
	// panic(fmt.Errorf("not implemented: SortTeamCollections - sortTeamCollections"))
	tx := r.DB.Begin()

	/* -- Team Collection -- */
	var collections []*model.TeamCollection
	base := GetPreloadedDB(tx, ctx).Where(
		`"teamID" = ? AND "parentID" IS ?`, teamID, parentCollectionID).Clauses(
		clause.Locking{Strength: clause.LockingStrengthUpdate})
	if sortOption == dto.SortOptionsTitleAsc {
		base = base.Order(`"Title" ASC`)
	} else if sortOption == dto.SortOptionsTitleDesc {
	...
	for i, coll := range collections {
		err := tx.Model(&model.TeamCollection{}).Where(`id = ?`, coll.ID).Update(`"orderIndex"`, i+1).Error
		if err != nil {
			tx.Rollback()
			return false, ex.ErrTeamColReorderingFailed
		}
	}
	...
	for i, req := range requests {
		err := tx.Model(&model.TeamRequest{}).Where(`id = ?`, req.ID).Update(`"orderIndex"`, i+1).Error
		if err != nil {
			tx.Rollback()
			return false, ex.ErrTeamReqReorderingFailed
		}
	}

	if parentCollectionID != nil {
		TeamChildCollectionsSortedSub.Publish(parentCollectionID)
	} else {
		TeamRootCollectionsSortedSub.Publish(true)
	}
	tx.Commit()
	return true, nil
}
...
func (r *queryResolver) IsSMTPEnabled(ctx context.Context) (bool, error) {
	// panic(fmt.Errorf("not implemented: IsSMTPEnabled - isSMTPEnabled"))
	InfraConfigsMap, err := GetInfraConfigsMap(r.DB)
	if err != nil {
		return false, err
	}
	return InfraConfigsMap[string(dto.InfraConfigEnumMailerSMTPEnable)] == "ENABLE", nil
}

func (r *queryResolver) IsUserHistoryEnabled(ctx context.Context) (*model.InfraConfig, error) {
	// panic(fmt.Errorf("not implemented: IsUserHistoryEnabled - isUserHistoryEnabled"))
	var config model.InfraConfig
	if err := r.DB.First(&config, "name=?", string(dto.InfraConfigEnumUserHistoryStoreEnabled)).Error; err != nil {
		return nil, err
	}
	return cast(config)
}
...
func (r *queryResolver) Infra(ctx context.Context) (*dto.Infra, error) {
	...
	var invitedUsers []*model.InvitedUser
	if err := r.DB.Find(&invitedUsers).Error; err != nil {
		return nil, err
	}
	return &dto.Infra{
		ExecutedBy:           admin,
...
func (r *queryResolver) AllowedAuthProviders(ctx context.Context) ([]string, error) {
	...
	allowedAuthProviders := strings.Split(allowedAuthProvider, ",")
	return allowedAuthProviders, nil
}
...
func (r *queryResolver) Admin(ctx context.Context) (*dto.Admin, error) {
	...
	return &dto.Admin{
		...
		LastLoggedOn: user.LastActiveOn,
...
func (r *queryResolver) ExportCollectionToJSON(ctx context.Context, teamID string, collectionID string) (string, error) {
	// panic(fmt.Errorf("not implemented: ExportCollectionToJSON - exportCollectionToJSON"))
	c := model.TeamCollection{}
	if err := GetPreloadedDB(r.DB, ctx).First(&c, "\"teamID\" = ? AND id = ?", teamID, collectionID).Error; err != nil {
		return "", err
	}
...
func (r *queryResolver) RootCollectionsOfTeam(ctx context.Context, cursor *string, take *int, teamID string) (c []*model.TeamCollection, err error) {
	...
	err = base.Limit(getLimit(take)).Find(&c, `"teamID" = ? AND "parentID" IS NULL`, teamID).Error
	return
}
...
func (r *queryResolver) ExportUserCollectionToJSON(ctx context.Context, collectionID string) (string, error) {
	// panic(fmt.Errorf("not implemented: ExportUserCollectionToJSON - exportUserCollectionToJSON"))
	user, ok := ctx.Value(mw.ContextKey("operator")).(*model.User)
	if !ok {
		return "", ex.ErrBugAuthNoUserCtx
	}
	c := model.UserCollection{}
	if err := GetPreloadedDB(r.DB, ctx).First(&c, "\"userUid\" = ? AND id = ?", user.UID, collectionID).Error; err != nil {
		return "", err
	}
	result, err := json.Marshal(c)
	return string(result), err
...
func (r *queryResolver) InfraTokens(ctx context.Context, skip *int, take *int) (tks []*model.InfraToken, err error) {
	// panic(fmt.Errorf("not implemented: InfraTokens - infraTokens"))
	base := GetPreloadedDB(r.DB, ctx)
	if skip != nil {
		base = base.Offset(*skip)
	}
	err = base.Limit(getLimit(take)).Find(&tks).Error
	return
}
...
func (r *subscriptionResolver) InfraConfigUpdate(ctx context.Context, configName dto.InfraConfigEnum) (<-chan string, error) {
	// panic(fmt.Errorf("not implemented: InfraConfigUpdate - infraConfigUpdate"))
	ch := make(chan string)
	go func() {
		defer close(ch)
		for e := range InfraConfigUpdateSub.Subscribe() {
			cfg := e.(*dto.InfraConfigArgs)
...
func (r *subscriptionResolver) UserHistoryAllDeleted(ctx context.Context) (<-chan bool, error) {
	// panic(fmt.Errorf("not implemented: UserHistoryAllDeleted - userHistoryAllDeleted"))
	ch := make(chan bool)
	go func() {
		defer close(ch)
		for e := range UserHistoryAllDeletedSub.Subscribe() {
			ch <- e.(bool)
		}
	}()
	return ch, nil
...
func (r *subscriptionResolver) UserCollectionDuplicated(ctx context.Context) (<-chan *dto.UserCollectionDuplicatedData, error) {
	// panic(fmt.Errorf("not implemented: UserCollectionDuplicated - userCollectionDuplicated"))
	ch := make(chan *dto.UserCollectionDuplicatedData)
	go func() {
		defer close(ch)
		for e := range userCollectionDuplicatedSub.Subscribe() {
			ch <- e.(*dto.UserCollectionDuplicatedData)
		}
	}()
	return ch, nil
}

func (r *subscriptionResolver) UserRootCollectionsSorted(ctx context.Context) (<-chan *dto.UserCollectionSortData, error) {
	// panic(fmt.Errorf("not implemented: UserRootCollectionsSorted - userRootCollectionsSorted"))
	ch := make(chan *dto.UserCollectionSortData)
	go func() {
		defer close(ch)
		for e := range UserRootCollectionsSortedSub.Subscribe() {
			ch <- e.(*dto.UserCollectionSortData)
		}
	}()
	return ch, nil
}

func (r *subscriptionResolver) UserChildCollectionsSorted(ctx context.Context) (<-chan *dto.UserCollectionSortData, error) {
	// panic(fmt.Errorf("not implemented: UserChildCollectionsSorted - userChildCollectionsSorted"))
	ch := make(chan *dto.UserCollectionSortData)
	go func() {
		defer close(ch)
		for e := range UserChildCollectionsSortedSub.Subscribe() {
			ch <- e.(*dto.UserCollectionSortData)
		}
	}()
	return ch, nil
...
func (r *subscriptionResolver) TeamRootCollectionsSorted(ctx context.Context, teamID string) (<-chan bool, error) {
	// panic(fmt.Errorf("not implemented: TeamRootCollectionsSorted - teamRootCollectionsSorted"))
	ch := make(chan bool)
	go func() {
		defer close(ch)
		for e := range TeamRootCollectionsSortedSub.Subscribe() {
			ch <- e.(bool)
		}
	}()
	return ch, nil
...
func (r *subscriptionResolver) TeamChildCollectionsSorted(ctx context.Context, teamID string) (<-chan string, error) {
	// panic(fmt.Errorf("not implemented: TeamChildCollectionsSorted - teamChildCollectionsSorted"))
	ch := make(chan string)
	go func() {
		defer close(ch)
		for e := range TeamChildCollectionsSortedSub.Subscribe() {
			ch <- e.(string)
		}
	}()
	return ch, nil
...
func (r *teamResolver) MyRole(ctx context.Context, obj *model.Team) (*model.TeamAccessRole, error) {
...
func (r *teamCollectionResolver) Children(ctx context.Context, obj *model.TeamCollection, cursor *string, take *int) (colls []*model.TeamCollection, err error) {
	// panic(fmt.Errorf("not implemented: Children - children"))
	// base := GetPreloadedDB(r.DB, ctx).Where(`"parentID"=?`, obj.ID)
	// if cursor != nil {
	// 	base = base.Where("id > ?", *cursor)
	// }
	// err = base.Limit(getLimit(take)).Find(&colls).Error
	// return
...
func (r *teamRequestResolver) Request(ctx context.Context, obj *model.TeamRequest) (string, error) {
	// panic(fmt.Errorf("not implemented: Request - request"))
...
func (r *userCollectionResolver) Requests(ctx context.Context, obj *model.UserCollection, cursor *string, take *int) (reqs []*model.UserRequest, err error) {
	// panic(fmt.Errorf("not implemented: Requests - requests"))
	err = GetPreloadedDB(r.DB, ctx).Find(&reqs, `"parentID"=?`, obj.ID).Error
	return
}
...
func (r *userRequestResolver) Request(ctx context.Context, obj *model.UserRequest) (string, error) {
	// panic(fmt.Errorf("not implemented: Request - request"))
	res, err := json.Marshal(obj.Request)
	return string(res), err
}

func (r *Resolver) Infra() InfraResolver { return &infraResolver{r} }

func (r *Resolver) InfraToken() InfraTokenResolver { return &infraTokenResolver{r} }
...
func (r *Resolver) TeamCollection() TeamCollectionResolver { return &teamCollectionResolver{r} }

func (r *Resolver) TeamRequest() TeamRequestResolver { return &teamRequestResolver{r} }

func (r *Resolver) User() UserResolver { return &userResolver{r} }

func (r *Resolver) UserCollection() UserCollectionResolver { return &userCollectionResolver{r} }

func (r *Resolver) UserRequest() UserRequestResolver { return &userRequestResolver{r} }

type infraResolver struct{ *Resolver }
type infraTokenResolver struct{ *Resolver }
type mutationResolver struct{ *Resolver }
type queryResolver struct{ *Resolver }
type subscriptionResolver struct{ *Resolver }
type teamResolver struct{ *Resolver }
type teamCollectionResolver struct{ *Resolver }
type teamRequestResolver struct{ *Resolver }
type userResolver struct{ *Resolver }
type userCollectionResolver struct{ *Resolver }
type userRequestResolver struct{ *Resolver }
